...
extern SemaphoreHandle_t servo_mutex; // Mutex para controlar o acesso ao servo
...
void task_self_test(void *params) {
    ...
    xSemaphoreGive(self_test_sem); // Libera a task_distancia_display
    xSemaphoreGive(self_test_sem); // Libera a task_alerta_proximidade
    ...
    if (xSemaphoreTake(self_test_sem, portMAX_DELAY) == pdTRUE) {
        ...
        while (true) {
            ...
            if (distancia_local <= 150 && !servo_foi_acionado) {
                ...
                // Tenta pegar o controle do servo, mas não bloqueia se estiver em uso
                if (xSemaphoreTake(servo_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
                    ...
                    xSemaphoreGive(servo_mutex); // Libera o controle imediatamente
                }
                servo_foi_acionado = true;
            } 
            else if (distancia_local < 100) { /* ...código de alerta inalterado... */ } 
...
void task_braco_controle(void *params) {
    ...
    bool tem_controle_servo = false; // Flag para saber se esta tarefa possui o mutex

    while (true) {
        bool btn_a_pressed = !gpio_get(BTN_A_PIN);

        if (btn_a_pressed) {
            // Tenta pegar o controle do servo se ainda não o tiver
            if (!tem_controle_servo) {
                if (xSemaphoreTake(servo_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
                    tem_controle_servo = true;
                }
            }
            ...
            // Se o botão foi solto, libera o controle do servo
            if (tem_controle_servo) {
                pwm_set_chan_level(slice_base, pwm_gpio_to_channel(SERVO_BASE_PIN), 0);
                xSemaphoreGive(servo_mutex);
                tem_controle_servo = false;
            }
        }
